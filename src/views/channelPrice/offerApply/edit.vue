<template>
  <div class="profitRate-page">
     ...
     <div style="text-align: left;margin-bottom: 10px;">报价明细</div>
      ...
      >
        ...
        <el-table-column  label="本次合作价" min-width="110px" >
              <template slot-scope="scope">
                    <div  v-if="scope.row.isEdit">
                        <el-input
                  v-model="scope.row.tempCurrentMsrp"
                  ...
                  onkeyup="this.value=this.value.replace(/[^\d.]/g,'');"
                    ...
                    <div v-else>
                      <span style="margin-left:10px;" >{{scope.row.tempCurrentMsrp}}</span>
                      <span style="float: right" @click="scope.row.isEdit=true"><el-link :underline="false" ><i class="el-icon-edit"></i></el-link></span>
                    </div>
        ...
        <el-table-column  label="操作">
           <template slot-scope="scope">
          <el-button  v-if="scope.row.isEdit"  v-permission-button="'edit'" @click="updateApplyGoods(scope.row)" type="text">保存</el-button>
          <el-button  v-if="scope.row.isEdit"  v-permission-button="'edit'" @click="cancelEdit(scope.row)" type="text">取消</el-button>
          <el-button    v-permission-button="'modifyLogs'"  @click="getModifyLogs(scope.row.id)" type="text">修改日志</el-button>
  ...
  <div slot="before-table">
      <div class="pb10 btn" style="margin:auto">
        <el-button  type="success"  @click="channelOfferApply()" style="margin-right:20px" v-permission-button="'approved'">提交审批</el-button>  
...
<script>
import {
getChannelOfferApply,getListGoods,updateApplyGoods,channelOfferApply,listApplyLog,getOfferModifyLogPages
} from '@/api/profitRate'
...
export default {
  ...
  methods: {
    ...
    async getListGoods() {
      getListGoods(this.applyId).then((res) => {
       ...
       this.tableData= res.map((item)=>{
           item.tempCurrentMsrp=item.currentMsrp
           item.isEdit=false
         return item
       })
        }
    ...
    cancelEdit(obj){
       obj.isEdit=false
       obj.tempCurrentMsrp=obj.currentMsrp
       this.calMsrp(obj)
    },
    updateApplyGoods(obj){
      if(!obj.tempCurrentMsrp){
        this.$message.error('商品合作价不能为空')
        return;
      }
      this.$store.commit("SHOW_LOADING");
      let  param={id:obj.id,currentMsrp:obj.tempCurrentMsrp}
      updateApplyGoods(param).then((res) => {
          obj.currentMsrp=obj.tempCurrentMsrp
          this.$message.success('商品合作价保存成功')
          this.$store.commit("SHOW_LOADING",false);
      }).catch(() => {
        this.$store.commit("SHOW_LOADING",false);
      }).finally(() => {
    ...
    calMsrp(obj){
      //成本价计算
      obj.currentProfitRates=!obj.tempCurrentMsrp||obj.tempCurrentMsrp==0?0:accSub(100,accMul(accDiv(obj.costPrice,obj.tempCurrentMsrp).toFixed(4),100).toFixed(2))
      obj.amplitude=accSub(obj.currentProfitRates,obj.channelProfitRates)
    },
    channelOfferApply(){
     if(this.isExistModify()){
        this.$message.error('定价申请单商品合作价未保存，请保存后再提审')
        return
     }
        this.$store.commit("SHOW_LOADING","提审中");
        channelOfferApply(this.applyId).then((res) => {
    ...
    getApplyLog(){
     ...
     isExistModify(){
       ...
       this.tableData.forEach(item=>{
         console.info("12:",item.tempCurrentMsrp+'|'+item.currentMsrp)
         if(item.tempCurrentMsrp!=item.currentMsrp){
           isExistModify= true;
         }
       })
       return isExistModify;
    },
    goBack(){
