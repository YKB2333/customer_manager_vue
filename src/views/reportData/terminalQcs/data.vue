    ...
    <my-table v-model="searchQuery" :searchForm="searchForm" @change="onChange">
      ...
      <div slot="before-table">
        <div class="pb10">
          ...
          <el-button v-permission-button="'uploda'" type="text" @click="showUploadAtta()" style="margin-right:200px">上传报表</el-button>
          <el-date-picker
      ...
      <el-table
        ...
        <el-table-column label="销售周" prop="week" width="100" ></el-table-column>
     ...
     <!-- <div v-show="docDialog.dialogType === 'attaList'">
      ...
      <div v-show="docDialog.dialogType">
        <el-upload
          ref="upload"
          class="inline-block"
          :action="`${$config.baseURL}/customer/report-qcs-data/uploadReport`"
...
import { getQcsReportList} from '@/api/reportData'
...
export default {
  ...
  methods: {
    ...
    onStartDateChange(value) {
      if (value){
        this.searchQuery.startWeek=`${value.getFullYear()}${this.$moment(value).week()}`
      }else {
        this.searchQuery.startWeek=0
      }
    },
    onEndDateChange(value) {
      if(value) {
        this.searchQuery.endWeek=`${value.getFullYear()}${this.$moment(value).week()}`
      }else {
        this.searchQuery.endWeek=1000000
      }
    },
    // 加载表格数据
    ...
    fetchBrandList() {
      return new Promise((resolve, reject) => {
        getQcsReportList(this.searchQuery)
          .then(res => {
            resolve(res)
          }).catch(error => {
          reject(error)
