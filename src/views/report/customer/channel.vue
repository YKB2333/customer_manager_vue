...
import { getChannelSales, getTotalAmt } from "@/api/report";
export default {
  name: "SalesReportByChannel",
  data() {
    return {
      tableLoading: false,
      query: {
  ...
  async created() {
    this.query.time[0] = this.$dayjs()
      .startOf("month")
      .valueOf();
    this.query.time[1] = this.$dayjs()
      .endOf("month")
      .valueOf();
