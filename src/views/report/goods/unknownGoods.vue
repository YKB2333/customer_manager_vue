...
import { getUnkonwGoods,getUnkonwGoodsTotalAmt } from "@/api/report";
export default {
  name: "SalesReportByUnkonwGoods",
  data() {
    return {
      tableLoading: false,
      query: {
  ...
  methods: {
    ...
    fetchTotalAmt() {
      ...
      params.startTime = params.time[0];
      params.endTime = params.time[1];
      return getUnkonwGoodsTotalAmt(params).then(res => {
        this.totalAmt = res;
      });
    },
    onChange() {
