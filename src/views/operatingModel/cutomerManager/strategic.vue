  ...
  <div>
    <my-table v-model="searchQuery" :searchForm="searchForm" @change="onChange">
      ...
      >
        ...
        <el-table-column prop="salesTarget" label="销售目标(万)"></el-table-column>
...
<!--        <el-table-column label="行动计划上传">
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template slot-scope="scope">
           ...
           <!-- <el-button type="text" @click="$router.push({name:'moving',query:scope.row})">行动计划管理</el-button>-->
          </template>
        </el-table-column>
      </el-table>
    </my-table>
    ...
    <my-dialog
      ...
      @close="closeDialog"
    >
      <el-form :model="saveForm" ref="saveForm" :rules="rules" :label-width="labelWidth" inline>
        <div>
        ...
        <div>
          <el-form-item label="销售目标(万)" prop="salesTarget">
            <el-input-number :controls="false" v-model="saveForm.salesTarget" placeholder="请输入销售目标" :style="inputStyle"></el-input-number>
          </el-form-item>
        </div>
        <div>
        ...
        <div>
          <el-form-item label="目标达成的行动计划" prop="attas">
            <file-list
              v-if="dialogVisible"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        businessId: [{ required: true, message: "请选择业务板块", trigger: "blur" }],
        salesTarget: [
          { required: true, message: "销售目标", trigger: "blur" }
        ],
        attas: [
          { required: true, message: "请上传目标达成的行动计划", trigger: "blur" }
        ],
        deptIds: [
          { required: true, message: "请选择制定部门", trigger: "blur" }
        ]
      },
    };
  },
  ...
  methods: {
    ...
    addAtta(atta) {
      console.log("a",atta)
      console.log(this.saveForm.attas)
