<template>
      <div>
        <el-upload ref="upload" class="inline-block"
          :action="action"
          ...
          <div v-else>
            <el-tag size='medium'  v-for="(item,index) in fileName" :key="index" closable @close="handleClose(item)" @click.stop="handleClick(item)">{{ item.fileName }}</el-tag>
...
<script>
import { uploadCustomerLogo,uploadCustomerAtta } from '@/api/client'
...
export default {
  ...
  watch:{
    isUrl:function(indexVal, oldVal){
      this.getIsUrl()
    }
  },
  methods: {
    ...
    // 点击
    handleClick(item){
      this.$emit('handleClick',item) //  文件点击
    },
    getIsUrl(){
    ...
    uploadRequest(data) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if(this.type){
          uploadCustomerLogo(formData).then((res)=>{
          ...
          /*
            文件多上传 把文件变成数组循环显示
          */ 
          uploadCustomerAtta(formData).then((res)=>{
              this.uploadLoading = false
